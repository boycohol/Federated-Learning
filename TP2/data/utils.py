...
def generate_data(
    ...
    for client_id in tqdm(range(n_clients), total=n_clients, desc="Clients.."):
        ...
        # Here, if each client only has at most 2 digits in its local dataset. That means the clients only been trained with different distributions of dataset. For example: client1 only train and test with digit 3-6,
        # client 2 only train and test with digit 1-0. So when the server aggreator the weight, because each client converge to their own optimal points but with different distribution, the aggreation will unstable,
        # which mean, the convergence to the global optimal point will be noisy and slow convergence, when the client get the average weight, it's also not match with client's distribtuion, so the loss increase again and 
        # it make the server, the aggreator hard to converge. 
